...
of transactions and blocks, just like other full nodes,
but additionally try to form new blocks containing
recently arrived transactions. There is a specific
mathematical puzzle that the miners need to solve in
order to produce a new block. If they solve this puzzle
correctly, the new block will be accepted by the network,
and the miner will be rewarded with some newly issued
bitcoin as a "finder's fee". This finder's fee, called
the **coinbase reward**, started out at 50 BTC per block,
